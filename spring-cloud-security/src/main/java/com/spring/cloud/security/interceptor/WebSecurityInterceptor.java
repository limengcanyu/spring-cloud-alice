...
//        String requestUUIDOfToken = tokenClaims.get(TokenConstant.TOKEN_NAME_REQUEST_UUID, String.class);
        ...
        // 获取Redis中token
        String loginUUIDOfRedis = redisTemplateUtils.getString(TokenConstant.REDIS_TOKEN_PREFIX_LOGIN_UUID + userId);
...
//        }
        ...
        if (!ObjectUtils.nullSafeEquals(UriConstant.URI_LOGOUT, requestUri)) {
            ...
            redisTemplateUtils.setString(TokenConstant.REDIS_TOKEN_PREFIX_LOGIN_UUID + userId, loginUUIDOfRedis, 2, TimeUnit.HOURS);
            logger.debug("安全拦截器 刷新登录token成功！");

            // 获取Redis中用户信息
            RedisPlatformUser loginUser = redisTemplateUtils.getObject(TokenConstant.REDIS_TOKEN_PREFIX_USER_INFO + userId, RedisPlatformUser.class);
